...
async function run() {
    ...
    if (!external_fs_.existsSync(targetPath)) {
        throw new Error(`No such file ${targetPath}.`);
    }
    const stat = external_fs_.statSync(targetPath, { throwIfNoEntry: true });
    if (stat === undefined) {
        throw new Error(`Can not access to ${targetPath}.`);
    }
    if (!stat.isFile() && !external_fs_.existsSync(external_path_.join(targetPath, 'package.json'))) {
        throw new Error(`No such file ${external_path_.join(targetPath, 'package.json')}.`);
    }
    const filePath = stat.isFile() ? targetPath : external_path_.join(targetPath, 'package.json');
    const absolutePath = external_path_.resolve(filePath);
    const workDir = external_path_.dirname(absolutePath);
    core.info(`work dir: ${workDir}`);
    const option = yarn === true ? optionMap.yarn : optionMap.npm;
    const packageJson = JSON.parse(external_fs_.readFileSync(absolutePath).toString());
    const { devDependencies, peerDependencies, optionalDependencies, dependencies, } = packageJson;
    const command = external_child_process_namespaceObject.execFileSync('which', [yarn === true ? 'yarn' : 'npm'], {
        cwd: workDir,
        windowsHide: true,
        shell: false,
    }).toString().trim();
    ...
    external_child_process_namespaceObject.execFileSync(command, ['install'], {
        cwd: workDir,
        windowsHide: true,
        shell: false,
    });
    upgradeArgs
        ...
        external_child_process_namespaceObject.execFileSync(command, args, {
            cwd: workDir,
            windowsHide: true,
            shell: false,
        });
